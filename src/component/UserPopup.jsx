    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 p-4">
      <div className="relative w-[520px] h-[620px] bg-white border border-[#BC9D72] rounded-xl p-2 shadow-lg overflow-y-auto">
        <h2 className="text-[18px] font-black text-[#837958] text-center mb-4 mt-4">
          {activeTab === "customers" &&
            (customerData.id ? "แก้ไขข้อมูลลูกค้า" : "เพิ่มข้อมูลลูกค้า")}
          {activeTab === "technicians" &&
            (technicianData.id
              ? "แก้ไขข้อมูลเจ้าหน้าที่"
              : "เพิ่มข้อมูลเจ้าหน้าที่")}
          {activeTab === "admin" &&
            (adminData.id ? "แก้ไขข้อมูลแอดมิน" : "เพิ่มข้อมูลแอดมิน")}
        </h2>
            ...
            <form onSubmit={handleSubmit}>
              {activeTab === "customers" && (
                <>
                  ...
                  <select
                    name="buildingName"
                    value={customerData.buildingName} // ใช้เฉพาะชื่ออาคารเป็น value
                    onChange={handleCustomerChange}
                    className="w-[320px] text-sm border border-[#837958] rounded-lg px-2 py-1.5 focus:outline-none"
                  >
                    <option value="">เลือกอาคาร</option>
                    {buildings.map((building) => (
                      <option
                        key={building.id}
                        value={building.buildingName}
                        data-id={building.id} // เก็บ ID ใน data attribute
                      >
                        {building.buildingName}
                      </option>
                    ))}
                ...
                <>
                  ...
                  {isTechnicianEditMode && (
                    <div className="mt-4 mb-4">
                      {" "}
                      {/* เพิ่ม margin บน/ล่างเพื่อให้มีช่องว่าง */}
                      <label className="text-[#BC9D72] text-[12px] font-medium block mb-2">
                        อาคาร
                      </label>
                      <div className="grid grid-cols-1 gap-3 max-h-[120px] overflow-y-auto pr-2">
                        {" "}
                        {/* ปรับ grid และเพิ่ม scrollbar */}
                        {buildings.map((building) => (
                          <label
                            key={building.id}
                          ...
                          >
                            ...
                            />
                            <span className="text-[#837958] text-sm font-medium">
                              {building.buildingName}
                            </span>
                          </label>
                        ))}
                ...
                >
                  ...
                  {activeTab === "technicians" &&
                    (technicianData.id
                      ...
                      : "เพิ่มข้อมูลเจ้าหน้าที่")}
                  {activeTab === "admin" &&
                    (adminData.id ? "บันทึกข้อมูลแอดมิน" : "เพิ่มข้อมูลแอดมิน")}
                </button>
                <button
                  type="button"
                  onClick={onClose}
...
const InputField = ({
  ...
  <div className="mb-2">
    <div className="flex items-center gap-1 mb-1">
      ...
      {note && (
        <span className="text-gray-500 text-[10px] font-normal">{note}</span>
      )}
    </div>
    <input
      type={type}
